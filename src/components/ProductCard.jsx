...
const ProductCard = ({ product, t, openModal }) => {
  ...
  let currentImageInfo =
    product.images.find(img => img.color === selectedColor && img.size === selectedSize)
        ...
        <div className="card-sizes">
          ...
          <div className="card-sizes-list">
            {product.sizes.map(size => (
              <button key={size} className={`size-button ${selectedSize === size ? 'active' : ''}`} onClick={() => handleSizeSelect(size)}>
                {size}
              </button>
            ))}
          </div>
